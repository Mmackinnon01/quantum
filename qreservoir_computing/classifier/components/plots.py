...
def plotReservoir(time_log: dict, off_diag=False, legend=True):
    title = "Density Matrix Time Evolution of Reservoir"
    if off_diag:
        fig, ax = plotOffDiagonalElements(time_log, title, legend)
    else:
        fig, ax = plotDiagonalElements(time_log, title, legend)
    return fig


def plotReservoirAndSystem(time_log: dict, off_diag=False, legend=True):
...
def plotDiagonalElements(time_log, title, legend=True):
    plt.style.use("default")

    fig, ax = plt.subplots(figsize=[15, 8])
    density_matrix_size = list(
        time_log.values())[0].matrix.shape[0]

    for i in range(density_matrix_size):
        ax.plot(
            list(time_log.keys()),
            [mat.matrix[i][i] for mat in list(time_log.values())],
...
def plotOffDiagonalElements(time_log, title, legend=True):
    plt.style.use("default")
    ...
    for i in range(density_matrix_size):
        for j in range(density_matrix_size):
            if i != j:
                line = np.abs(np.array([mat.matrix[i][j]
...
def plotExcitations(time_log, legend=True):
    plt.style.use("default")
...
def plotTotalExcitations(time_log, legend=True):
    plt.style.use("default")
...
def plotSigmaCombinations(time_log, legend=True):
    plt.style.use("default")
